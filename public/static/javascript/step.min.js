$(document).ready(function () {
  // Get the language cookie value, default to 'en' if not set
  var language = $.cookie("language") || "en";

  // Set the selected option based on the cookie value
  $("#language").val(language);

  // Handle the change event and update the cookie when a new language is selected
  $("#language").on("change", function () {
    var selectedLanguage = $(this).val();
    window.location.reload();
    $.cookie("language", selectedLanguage, { path: "/", expires: 30 }); // Set cookie with 30-day expiration
  });
});
  ...
  _createClass(stepsDemo, [
    ...
    },
    {
      ...
      value: function validateBy(trigger) {
        ...
        var from = isNextBtn
          ? null
          : $(trigger).parents("ul").children(".active").index();
        var to = isNextBtn ? null : $(trigger).parent().index();
        var $trigger = isNextBtn
          ? $(trigger)
          : $(trigger).parents("ul").children(".active");
        var group = $trigger.data().validate;
        var groupId = $trigger.parents(".content").attr("id");
        var $groupStep = isNextBtn
          ? $('[data-target="#'.concat(groupId, '"]'))
          : $trigger;
        $("#stepper-form")
          .parsley()
          .on("form:validate", function (formInstance) {
            ...
            if (isValid) {
              ...
              if ($trigger.hasClass("submit")) {
                ...
                CurlHttp(
                  $("#stepper-form").attr("action"),
                  $("#stepper-form").attr("method"),
                  $("#stepper-form").serializeArray(),
                );
              } else if (isNextBtn) {
                stepperDemo.next();
              } else {
              ...
              document.querySelector("#stepper").addEventListener(
                "show.bs-stepper",
                function (e) {
                  e.preventDefault();
                },
